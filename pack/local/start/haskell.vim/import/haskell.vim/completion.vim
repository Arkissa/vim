...
def ModulePrefix(base: string): string
	var idx = base->strridx('.')
	if idx < 0
		return ''
	endif

	return base->strpart(0, idx + 1)
enddef

def StripModulePrefix(item: string, base: string): string
	var prefix = ModulePrefix(base)

	if prefix !=# '' && item->stridx(prefix) == 0
		return item->strpart(prefix->strlen())
	endif

	return item
enddef

def ToImportItems(items: list<string>, base: string): list<dict<any>>
	return items->mapnew((_, item) => {
		return {
			word: item,
			abbr: StripModulePrefix(item, base)}
	})
enddef
...
class MetaGHCLanguageExtensions implements MetaCompleter
	...
	def RuleOk(line: string): bool
		return line =~ '^\s*{-#\s*LANGUAGE.\{-}}'
	enddef

	def Start(token: string): number
		return token->match('[, ]\zs[^, ]\{-}$')
	enddef
endclass

class GHCOptions implements Completer
	def Complete(base: string): any
		return base == ""
			? optionsGHC
			: optionsGHC
...
class HaskellModules implements Completer
	...
	def Complete(base: string): any
		...
		if type(response) != v:t_list || response->empty()
			...
			this._cache = ToImportItems(response, base)->matchfuzzy(base, {key: 'word'})
		endif
...
class MetaHasakellModules implements MetaCompleter
	...
	def RuleOk(line: string): bool
		return line =~ '^\s*import\s*.*'
	enddef

	def Start(token: string): number
		return token->match('\s\zs[^ ]\{-}$')
	enddef
endclass
...
class HaskellLexerToken implements Completer
	...
	def Complete(base: string): any
		...
		return response->matchfuzzy(base)
	enddef
endclass
...
class MetaHasekellLexerToken implements MetaCompleter
	...
	def Start(token: string): number
		return token->match('\k\{-}$')
	enddef
endclass

def MetaCompleters(): list<MetaCompleter>
	return [
		MetaGHCOptions.new(),
		MetaGHCLanguageExtensions.new(),
		MetaHasakellModules.new(),
		MetaHasekellLexerToken.new()
	]
enddef
...
export class HaskellComplete extends CompleteFunc
	...
	def First(): number
		...
		if synAttr =~# 'String\|Comment'
			return -3
		endif

		var line = getline('.')
		var start = charcol('.')
		var token = line->strcharpart(0, start)
		for meta in MetaCompleters()
			if meta.RuleOk(line)
				start = meta.Start(token)
				if start < 0
					break
				endif
	...
	def Complete(base: string): any
		...
		return this._completer.Complete(base)
	enddef
endclass
